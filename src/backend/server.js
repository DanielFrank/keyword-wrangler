      ...
      POST: function (req,res){
        req.onJson(function(err, newKeyword){
          if (err) {
            ...
            dbSession.fetchAll('INSERT INTO keyword (value, categoryID) VALUES(?, ?)',
  ...
  server.route('/api/keywords/:id',
    {
        ...
        req.onJson(function(err, keyword){
          if (err) {
            ...
            dbSession.query('UPDATE keyword SET value=?, categoryID=? WHERE keyword.id=?',
              [keyword.value, keyword.categoryID, keywordId], function(err,results){
                if(err) {
                  console.log(err);
                  res.status.internalServerError(err);
                } else {
      ...
      DELETE: function (req,res){
        ...
        dbSession.query('DELETE FROM keyword WHERE keyword.id=?',
          [keywordId], function(err,results){
            if(err) {
              console.log(err);
              res.status.internalServerError(err);
            } else {
